\section*{Traditional Network Utilities} % ## Basic network utilities

In the world of \cmd{TCP/IP} Network, other programs have been developed that
...
you can use the lighter `\cmd{nc}' utility, \cmd{netcat}. For example let's
try to play with \cmd{HTTP} protocol:
\begin{code}{mverb}
...
\cmd{FTP} or \struct{File Transfer Protocol} is another well-known part of
...
through a regular web browser as well as through the \cmd{ftp} utility:
\begin{code}{mverb}
man ftp
\end{code}
...
you can download file by `\cmd{get}' command and upload by `\cmd{put}',
but only a single file. If you want to work with multiple files,
you will need to use the `\cmd{mget}'/`\cmd{mput}' commands.

In this case, it makes sense to disable the questions about confirming
operations using the \cmd{prompt} command. Also switching to binary mode using
the \cmd{bin} command can be important for the Microsoft client system.
...
.
\end{code} % $
The mail message must end with one `\cmd{dot}' per line.
